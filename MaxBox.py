...
def BoxCons(inputs,hull):
    ...
    for i in range(0,2):# i denotes forward or aft corner
        for j in range(0,2):  # j denotes upper (Dd) or lower corner (Dd-h)            
            ...
            elif inputs[i] < Pos[j,0]:#next check if the position is in the bow profile
                if inputs[i] < hull.bow_profile(depths[j]): #check if the position is fwd of the bow profile
                    y = 0.0
                else:
                    y = hull.halfBeam_Bow([inputs[i]],hull.solve_waterline_bow(depths[j])) #assign it to the halfbeam at that position otherwise
